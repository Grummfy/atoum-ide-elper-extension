...
namespace atoum\ideHelper\tests\units\report\fields\runner\failures\execute;
...
class phpstorm extends atoum\test
{
	...
	{
		$this
			->if($this->function->php_uname = 'Linux', $this->newTestedInstance($command = uniqid()))
			->then
				->string($this->testedInstance->getCommand())->isEqualTo($command . ' --line %2$d %1$s &> /dev/null &')
			->if($this->function->php_uname = 'Linux', $this->newTestedInstance)
			->then
				->string($this->testedInstance->getCommand())->isEqualTo('pstorm --line %2$d %1$s &> /dev/null &')
			->if($this->function->php_uname = 'Darwin', $this->newTestedInstance)
			->then
				->string($this->testedInstance->getCommand())->isEqualTo('/Applications/PhpStorm.app/Contents/MacOS/webide --line %2$d %1$s &> /dev/null &')
			->if($this->function->php_uname = 'FreeBSD', $this->newTestedInstance)
			->then
				->string($this->testedInstance->getCommand())->isEqualTo('pstorm --line %2$d %1$s &> /dev/null &')
			->if($this->function->php_uname = 'Windows')
				->exception(function()
				{
					$this->newTestedInstance();
				})
					->isInstanceOf(\RuntimeException::class)
					->message->isEqualTo('You must defined a path to phpstorm app');
		;
	}
	...
	{
		$this
			->if($this->function->php_uname = 'Linux', $this->newTestedInstance(uniqid()))
			->then
